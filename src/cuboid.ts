...
export class Cuboid {
	...
	public constructor(x1: number, x2: number, y1: number, y2: number, z1: number, z2: number) {
    ...
    */
	...
	public intersect(other: Cuboid): Cuboid {
		let d0 = Math.max(this.minX(), other.minX())
		let d1 = Math.max(this.minY(), other.minY())
		let d2 = Math.max(this.minZ(), other.minZ())
		let d3 = Math.min(this.maxX(), other.maxX())
		let d4 = Math.min(this.maxY(), other.maxY())
		let d5 = Math.min(this.maxZ(), other.maxZ())
		return new Cuboid(d0, d1, d2, d3, d4, d5)
	}

	public union(other: Cuboid): Cuboid {
		let d0 = Math.min(this.minX(), other.minX())
		let d1 = Math.min(this.minY(), other.minY())
		let d2 = Math.min(this.minZ(), other.minZ())
		let d3 = Math.max(this.maxX(), other.maxX())
		let d4 = Math.max(this.maxY(), other.maxY())
		let d5 = Math.max(this.maxZ(), other.maxZ())
		return new Cuboid(d0, d1, d2, d3, d4, d5)
	}
	...
	public getAverageEdgeLength(): number {
		let d0 = this.maxX() - this.minX()
		let d1 = this.maxY() - this.minY()
		let d2 = this.maxZ() - this.minZ()
		return (d0 + d1 + d2) / 3.0
	}
    ...
    getCenter(): Vector {
        return new Vector(this.minX() + (this.maxX() - this.minX()) * 0.5, this.minY() + (this.maxY() - this.minY()) * 0.5, this.minZ() + (this.maxZ() - this.minZ()) * 0.5);
    }
